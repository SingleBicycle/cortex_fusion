...
class SurfaceSubjectDataset(Dataset):
    ...
    def __init__(
        ...
        # Preload edge_index tensors for only needed resolutions.
        needed_res = set()
        if self.random_resolution:
    ...
    def _build_class_names(self) -> List[str]:
        annot_paths = sorted(set(self.df["annot_path"].astype(str).tolist()))
        names_set = set()
    ...
    def _load_hemi(self, row: Dict, row_res: str) -> Dict:
        ...
        if pial_faces.shape != white_faces.shape or not np.array_equal(pial_faces, white_faces):
            raise ValueError(f"faces topology mismatch: {row['sid']} {row['hemi']} {row_res}")

        n_verts = pial_xyz.shape[0]

        labels, ctab, names = read_annot(str(row["annot_path"]))
        del ctab
